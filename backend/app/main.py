...
if BackboardAnalyzer is None and _backboard_err is not None:
    logger.warning("Could not import BackboardAnalyzer: %s", _backboard_err)
if NoirVisionAnalyzer is None and _noirvision_err is not None:
    logger.warning("Could not import NoirVisionAnalyzer: %s", _noirvision_err)
...
try:
    ...
    noirvision = NoirVisionAnalyzer() if NoirVisionAnalyzer else None
...
async def analyze_complete(
    ...
    try:
        ...
        logger.error("Analysis failed: %s", type(e).__name__ + ": " + str(e))
        raise HTTPException(
            status_code=500,
